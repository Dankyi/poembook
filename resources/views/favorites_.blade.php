        ...
        <div class="flex flex-col break-words bg-blue-200 border border-2 rounded shadow-2xl">
            ...
            <div class="w-full p-6">
                <p class="text-justify whitespace-pre-line">
                    @if(strlen($favoritePoems->content) > 250)
                        {{ substr($favoritePoems->content, 0, 250) }} <a class="font-bold mb-2 text-blue-900" href="poem/{{ $favoritePoems->poem_id }}"> ... Read More</a>
                    @else
                        {{ $favoritePoems->content }}
                    @endif
                </p><br/><br/>

                <p>
                <h3 class="font-bold mb-2">Written By: {{ $favoritePoems->name }}</h3>
                </p>

                <p>
                <h3>Published: {{ date('j M Y, H:i', strtotime($favoritePoems->created_at)) }}</h3>
                </p>
            </div>
        </div><br/><br/><br/><br/>
    </div>
